...
import com.joko.movie2021.core.extensions.firstOrDefault
import com.joko.movie2021.repository.data.remote.service.account.*
...
import com.joko.movie2021.repository.toAccountState
import com.joko.movie2021.repository.toActor
import com.joko.movie2021.repository.toMovie
import com.joko.movie2021.repository.toMovieTrailer
            ...
            .flatMap { accountStateResponse ->
                Single.just(
                    when (accountStateResponse) {
                        ...
                        is NetworkResponse.NetworkError -> {
                            Resource.Error(
                                accountStateResponse.error.localizedMessage ?: "Network Error"
                            )
                        }
                    }
                )
        ...
        return movieService.getVideosForMovie(movieId)
            ...
            .flatMapPublisher { movieVideosResponse ->
                Flowable.just(when (movieVideosResponse) {
                    ...
                    is NetworkResponse.NetworkError -> {
                        Resource.Error(
                            movieVideosResponse.error.localizedMessage ?: "Network Error"
                        )
                    }
                })
            }
            ...
            .flatMap { similarMoviesResponse ->
                Single.just(
                    when (similarMoviesResponse) {
                        ...
                        is NetworkResponse.NetworkError -> {
                            Resource.Error(
                                similarMoviesResponse.error.localizedMessage ?: "Network Error"
                            )
                        }
                    }
                )
